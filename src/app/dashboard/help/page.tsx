...
import { 
  ...
  LifeBuoy, ChevronRight, Zap, ExternalLink 
} from "lucide-react";
...
export default function HelpPage() {
  return (
    <div className="flex flex-col min-h-screen" style={{ background: "var(--background)" }}>
      ...
      <div className="flex-1 p-8 max-w-5xl mx-auto w-full animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="text-center mb-16">
        <div className="flex items-center justify-center gap-2 mb-4">
          <div className="w-8 h-8 rounded-lg bg-indigo-500/10 flex items-center justify-center">
            <LifeBuoy size={16} className="text-indigo-500" />
          </div>
          <span className="text-[11px] font-black uppercase tracking-[0.2em] text-indigo-500 opacity-80">Support Center</span>
        </div>
        <h1 className="text-6xl font-black tracking-tight font-plus-jakarta leading-none mb-6">
          How can we help?
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
        {CATEGORIES.map((cat, i) => (
          <Card key={i} className="p-6 border-none shadow-xl hover:shadow-2xl transition-all cursor-pointer group hover:-translate-y-1 duration-300" style={{ background: 'var(--card)' }}>
            <div 
            ...
            <h3 className="font-black text-lg mb-1">{cat.title}</h3>
            <p className="text-xs text-muted-foreground font-bold uppercase tracking-widest">{cat.count} Articles</p>
          </Card>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
        <div className="md:col-span-2 space-y-8">
          <h3 className="text-2xl font-black mb-6">Popular Questions</h3>
          <div className="space-y-4">
            {FAQS.map((faq, i) => (
              <Card key={i} className="p-6 border-none shadow-lg hover:shadow-xl transition-all cursor-pointer group" style={{ background: 'var(--card)' }}>
                <div className="flex items-start justify-between">
                  <div>
                    <p className="font-bold text-[17px] mb-2 group-hover:text-indigo-500 transition-colors">{faq.q}</p>
        ...
        <div className="space-y-6">
          <Card className="p-8 border-none bg-indigo-600 text-white relative overflow-hidden group">
            <MessageSquare size={80} className="absolute -right-8 -bottom-8 opacity-10 group-hover:scale-110 transition-transform" />
            <h4 className="text-xl font-black mb-2">Speak to Us</h4>
 ...
 <Button className="w-full font-black bg-white text-indigo-600 hover:bg-slate-100 shadow-xl transition-all">
          ...
          <Card className="p-8 border-none bg-muted/30 border border-border/20">
            <h4 className="font-black text-lg mb-4">Official Documentation</h4>
